...
<div class="list">
    ...
    <div class="listlf">
        <dl class="tit clear"><span><a href="<?php echo ($root); ?>index.php?s=tag-show-id-<?php echo (urlencode($tag_name)); ?>-sid-1-o-hits.html">按人气排序</a> <a href="<?php echo ($root); ?>index.php?s=tag-show-id-<?php echo (urlencode($tag_name)); ?>-sid-1-o-up.html">按投票排序</a></span><dt><a href="<?php echo ($InstallDir); ?>">首页</a> &raquo; TAG：<?php echo ($tag_name); ?></dt></dl>
        <ul><?php $tag_list=$pplist->ppvod('num:12;page:true;action:tag;order:vod_addtime desc');$page=$tag_list[0]['page']; ?>
        	<?php if(is_array($tag_list)): $i = 0; $__LIST__ = $tag_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ppvod): ++$i;$mod = ($i % 2 )?><li> <a href="<?php echo ($ppvod["vod_url"]); ?>" target="_blank"> <img src="<?php echo (getpicurl($ppvod["vod_pic"])); ?>" alt="<?php echo ($ppvod["vod_name"]); ?>" onerror="javascript:this.src='__PUBLIC__/images/nophoto.jpg';" width="110" height="150" border="0"/></a>
            ...
            <p>状态：<?php if(($ppvod["vod_continu"])  !=  "0"): ?><font color="red">连载至<?php echo ($ppvod["vod_continu"]); ?>集</font><?php else: ?>完结<?php endif; ?></p></li><?php endforeach; endif; else: echo "" ;endif; ?>
        </ul>
